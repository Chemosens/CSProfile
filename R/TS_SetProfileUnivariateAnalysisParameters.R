	...
	} else
	{
		for(att in levels(profileObject[["Attributes"]]))
		{
			if(model=="TwoWayMultiplicative" && nlevels(factor(profileObject[["Replicates"]]))<2)
			{
				...
				stop("The number of replicates is unsufficient for the Two-Way Multiplicative model. The model chosen is Two Way Additive")
				profileObject[["ANOVAModel"]]=model
			}
			...
			if(sum(!is.na(X[,"Y"]))>0)
			{
				...
				{
				# nombre d'observation par sujet et par produit
