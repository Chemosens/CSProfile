...
{ 
		...
		if (length(listAttributes)==0)
		 {
			 if(type=="html")
			 {
				 ...
				 html=("<html><body>")
				 html=paste(html, "<h2>",TS_GetLabel("FlashTable", language), "</h2>")
				 html=paste(html, "<p>",TS_GetLabel("NoSignificantAttribute", language),"</p>")
	...
	html=("<html><body>")
	html=paste(html, "<h2>",TS_GetLabel("FlashTable", language), "</h2>")
	html=paste(html, "<p>",TS_GetLabel("Similarity", language),": ", similarity, "</p>")
	html=paste(html, "<p>",TS_GetLabel("ClassificationMethod", language),": ", classificationMethod, "</p>")
	html=paste(html, "<p>",TS_GetLabel("MinimumProportionOfVarianceExplained", language),": ", explainedVariance, "</p>")
	html=paste(html, "<p>",TS_GetLabel("ContrastTestAlpha", language),": ", alphaContrast, "</p>")
	html=paste(html, "<p>",TS_GetLabel("VariableSelectionAlpha", language),": ",profileObject$alphaForSelectionOfSignificantAttributes , "</p>")
	html=paste(html, "<p>",TS_GetLabel("Model", language),": ", model, "</p>")
	html=paste(html, MatrixToHtml(flash,style=style), sep="")
	html=paste(html,"<p style='color:green'>",TS_GetLabel("FlashTableLegendGreen",language),"</p>",sep="")
	html=paste(html,"<p style='color:yellow'>",TS_GetLabel("FlashTableLegendYellow",language),"</p>",sep="")
	html=paste(html,"</body></html>",sep="",collapse="")		
