...
#'@param subject name of the column containing subjects. SubjectCode by default
#'@param rep name of the column containing replicates.Replicate by default
#'@param product name of the column containing products. ProductCode by default
#'@param attribute if format=="long", name of the column containing the attributes. AttributeCode by default
#'@param score if format=="long", name of the column containing the scores. Score by default
#'@param attributes if format=="wide", the attributes to be analyzed can be specified in this vector of characters. If NULL (default) all the columns that are not subject, product or rep are selected
#'@param format "wide" or "long". If long, the file or allData should contain only five columns: product, subject, rep, attribute and score
#'@export
#'@importFrom utils read.csv
#'@importFrom grDevices rainbow
profileReadData = function(file=NULL,allData=NULL, replaceNA="crossmean",
                           sep=";",transformSessionIntoReplicate=TRUE, percentageOfSubjectsByReplicate=0,
                           ...
                           format="long",subject="SubjectCode",product="ProductCode",rep="Replicate",attributes=NULL,attribute="AttributeCode",score="Score"
)
{
# Anciens defauts :percentageOfSubjectsByReplicate=0.75, percentageOfReplicatesBySubject=0.5,percentageOfProductBySubject=0.5
	...
	if(!rep%in% colnames(allData))
	{print("in");
	  warning("No replicate in this profile dataset.An additional column with Replicate = 1 was created");
	  allData[,"Replicate"]=1;
	  rep="Replicate"
	}
	...
	if(format=="wide")
	{
	  if(is.null(attributes)){attributes=colnames(allData)[!colnames(allData)%in%c(product,subject,rep)]}else{
	    if(any(!attributes%in%colnames(allData))){stop(paste0(attributes[!attributes%in%colnames(allData)]," is not column names of your data")) }
	  }
	  data=allData[,c(product,subject,rep,attributes)]
	  colnames(data)[1:3]=c("ProductCode","SubjectCode","Replicate")
	  allData=reshape(data,varying=list(attributes),direction="long",times=attributes,timevar="AttributeCode",v.names="Score")
	}

	# Selection des donnees de type Profile, format canonique
